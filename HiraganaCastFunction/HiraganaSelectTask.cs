...
public class HiraganaSelectTask
{
    ...
    private string ShiinnRegexSearch(string shiinnText, string boinnText, int boinnNumber)
    {
        ...
        {
            {RegexClass.exceptingN, HiraganaIndex.ltu + ShiinnIchimoji(shiinnText.Remove(0, 1), boinnText, boinnNumber)},
            {RegexClass.startStringTs, HiraganaIndex.ts[boinnNumber]}
        };
    ...
    public string ShiinnSanmoji(string shiinnText, string boinnText, int boinnNumber)
    {
        ...
        //"xtsu"("っ")の時のみ該当する
        if (shiinnText == RegexClass.startStringXts) { hiraganaText = HiraganaIndex.xtsu; }
